...
def clasificacion_binaria(random_state, model, grid, validacion_grid, grid_n_iter, random_grid, X_train, X_val, y_train_class3, y_val_class3, ensemble):
        ...
        precision = precision_score(y_val_class3, y_pred_class3, average='binary')
        print(f'📈 Precision (validacion): {precision:.4f}')
        recall = recall_score(y_val_class3, y_pred_class3, average='binary')
        print(f'📈 Recall (validacion): {recall:.4f}')
        f1 = f1_score(y_val_class3, y_pred_class3, average='binary')
...
def clasificacion_multiclase_categoria(model, X_train, X_val, y_train_class3, y_train_class2 , y_val_class2, y_pred_class3):
        
        indices_anomalia_train = np.where(y_train_class3.values == 1)[0]
        X_train = X_train.iloc[indices_anomalia_train]
        y_train_class2 = y_train_class2.iloc[indices_anomalia_train]
        y_train_class2 = y_train_class2.values.ravel()
        
        indices_anomalia_prediccion = np.where(y_pred_class3 == 1)[0]
        X_val = X_val.iloc[indices_anomalia_prediccion]
        y_val_class2 = y_val_class2.iloc[indices_anomalia_prediccion]
        y_val_class2 = y_val_class2.values.ravel()
        
        pesos = compute_class_weight('balanced', classes= np.array(list(anomalias.keys())), y=y_train_class2)
        ...
        indices_anomalias_reales = np.where(np.isin(y_val_class2, list(anomalias.keys())))[0]
        y_val_class2_reales = y_val_class2[indices_anomalias_reales]
        y_pred_class2_reales = y_pred_class2[indices_anomalias_reales]
        
        accuracy = accuracy_score(y_val_class2_reales, y_pred_class2_reales)
        print(f'📈 Accuracy (validacion): {accuracy:.4f}')
        precision = precision_score(y_val_class2_reales, y_pred_class2_reales, average='macro', zero_division=0)
        print(f'📈 Precision (validacion): {precision:.4f}')
        recall = recall_score(y_val_class2_reales, y_pred_class2_reales, average='macro')
        print(f'📈 Recall (validacion): {recall:.4f}')
        f1 = f1_score(y_val_class2_reales, y_pred_class2_reales, average='macro')
        print(f'📈 F1 (validacion): {f1:.4f}')
        
        return pipeline, y_pred_class2

def clasificacion_multiclase_tipo(model, X_train, X_val, y_train_class3, y_train_class1, y_val_class1, y_pred_class3, y_pred_class2, y_train_class2 , y_val_class2):
        
        pipeline_tipos = {}
        
        indices_anomalia_train = np.where(y_train_class3.values == 1)[0]
        X_train = X_train.iloc[indices_anomalia_train]
        y_train_class1 = y_train_class1.iloc[indices_anomalia_train].values.ravel()
        y_train_class2 = y_train_class2.iloc[indices_anomalia_train].values.ravel()
        
        indices_anomalia_prediccion = np.where(y_pred_class3 == 1)[0]
        X_val = X_val.iloc[indices_anomalia_prediccion]
        y_val_class1 = y_val_class1.iloc[indices_anomalia_prediccion].values.ravel()
        y_val_class2 = y_val_class2.iloc[indices_anomalia_prediccion].values.ravel()
...
def main(random_state, model, grid, validacion_grid, grid_n_iter, random_grid, ensemble, model_class2, model_class1):  
    ...
    pipeline_class2, y_pred_class2 = clasificacion_multiclase_categoria(model_class2, X_train, X_val, y_train_class3, y_train_class2 , y_val_class2, y_pred_class3)
    pipelines_class1 = clasificacion_multiclase_tipo(model_class1, X_train, X_val, y_train_class3, y_train_class1 , y_val_class1, y_pred_class3, y_pred_class2, y_train_class2 , y_val_class2)
