...
import numpy as np
import pandas as pd
from sklearn.metrics import ConfusionMatrixDisplay, accuracy_score, classification_report, confusion_matrix, f1_score, precision_score, recall_score, roc_auc_score, roc_curve
...
def guardar_conf(pre_path, metrics_class3, metrics_class2, metrics_class1, cm_df, class_df, cm_dfs, class_dfs):
    
    resumen = crear_resumen(metrics_class3, metrics_class2, metrics_class1, cm_df, class_df, cm_dfs, class_dfs)
...
def crear_resumen(metrics_class3, metrics_class2, metrics_class1, cm_df, class_df, cm_dfs, class_dfs):
    ...
    resumen += formatear_metricas("Clasificación Binaria", metrics_class3)
...
def clasificacion_binaria(y_test_class3, X_test, model):
    print("🔮 Clasificación binaria...")    

    y_pred_class3 = model.predict(X_test)
    ...
    predicciones_df.to_csv(os.path.join(ruta_directorio, 'arq1.csv'), index=False)
    
    metrics = [accuracy, precision, recall, f1]
    
    return metrics, y_pred_class3
 
def clasificacion_multiclase_categoria(y_pred_class3, y_test_class2, X_test, model_class2):
        ...
        indices_anomalias_reales = np.where(np.isin(y_test_class2, list(anomalias.keys())))[0]
        ...
        labels = sorted(set(y_test_class2_real))
        label_names = [str(label) for label in labels]  
        cm = confusion_matrix(y_test_class2_real, y_pred_class2_real)
        cm_df = pd.DataFrame(cm, index=[f'{label}' for label in label_names], columns=[f'{label}' for label in label_names])
        print(cm_df)
        class_df = classification_report(y_test_class2_real, y_pred_class2_real, target_names=label_names, digits=10)
        print(class_df)

        
        ruta_directorio = 'predicciones'
        predicciones_df = pd.read_csv(os.path.join('predicciones', 'arq1.csv'))
        predicciones_df['Predicciones_Class2'] = "Normal"  # Inicializa la columna con Normal
        predicciones_df.loc[indices_anomalia_test, 'Predicciones_Class2'] = y_pred_class2  # Asigna las predicciones
        os.makedirs(ruta_directorio, exist_ok=True)
        predicciones_df.to_csv(os.path.join(ruta_directorio, 'arq1.csv'), index=False)
        y_pred_class2_con_normales = np.full_like(y_pred_class3, "Normal", dtype=object)
        y_pred_class2_con_normales[indices_anomalia_test] = y_pred_class2
        
        metrics = [accuracy, precision, recall, f1]
        return metrics, y_pred_class2_con_normales, cm_df, class_df
 
def clasificacion_multiclase_tipo(y_pred_class3, y_pred_class2, y_test_class1, X_test, models_class1):
    ...
    ruta_directorio = 'predicciones'
    predicciones_df = pd.read_csv(os.path.join(ruta_directorio, 'arq1.csv'))
...
def main(model, path, model_class2, models_class1):  
    ...
    metrics_class3, y_pred_class3 = clasificacion_binaria(y_test_class3, X_test_processed, model)
    metrics_class2, y_pred_class2, cm_df, class_df = clasificacion_multiclase_categoria(y_pred_class3, y_test_class2, X_test_processed, model_class2)
    metrics_class1, y_pred_total, cm_dfs, class_dfs = clasificacion_multiclase_tipo(y_pred_class3, y_pred_class2, y_test_class1, X_test_processed, models_class1)
    
    guardar_conf(path, metrics_class3, metrics_class2, metrics_class1, cm_df, class_df, cm_dfs, class_dfs)
